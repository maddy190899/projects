...
export const ServicesPage = ({ setActivePage, onScopeSelected }) => {
  const PROCESS_STEPS = [
    ...
    },
    {
      ...
      desc: 'Developing on React 19 / Next.js 15 with zero placeholders, handcrafted SVG assets, and custom WebGL shaders for tactile depth.',
    },
    {
      step: '04',
    ...
    <div className="space-y-32 pt-28 sm:pt-36 px-6 md:px-12 max-w-7xl mx-auto">
      ...
      <div className="space-y-8">
        {SERVICES.map((service) => (
          <div
            key={service.id}
            className="p-8 sm:p-12 rounded-3xl bg-canvas-card border border-border-subtle hover:border-border-focus transition-all duration-300 shadow-card-elevated"
          >
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
              <div className="lg:col-span-4">
                <span className="text-2xl font-mono text-accent-volt font-bold block mb-2">
                  {service.number}
                </span>
                <h2 className="type-h3 font-display font-bold text-text-primary mb-4">
                  {service.title}
                </h2>
                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-accent-volt/10 text-accent-volt border border-accent-volt/20 text-xs font-mono">
              ...
              <div className="lg:col-span-8 space-y-6">
                <p className="text-text-secondary type-body leading-relaxed">
                  {service.description}
                </p>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {service.deliverables.map((item, idx) => (
                      <div
                        key={idx}
                        className="flex items-center gap-3 p-3.5 rounded-xl bg-canvas-surface border border-border-subtle text-xs font-mono text-text-primary"
                      >
                        <CheckCircle2 className="w-4 h-4 text-accent-volt flex-shrink-0" />
                        <span>{item}</span>
                      </div>
                    ))}
                  </div>
      ...
      <section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {PROCESS_STEPS.map((step, i) => (
            <div
              key={i}
              className="p-6 rounded-3xl bg-canvas-card border border-border-subtle shadow-inner-bevel flex flex-col justify-between"
            >
              <div>
                <span className="font-mono text-accent-volt text-lg font-bold block mb-4">
                  {step.step}
                </span>
